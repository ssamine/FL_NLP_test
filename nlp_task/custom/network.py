...
class NLPClinicalBert(nn.Module):
    def __init__(self, model_state_dict='/home/localuser/custom/model/pretraining', num_labels=1):
        super(NLPClinicalBert, self).__init__()
        
        self.model_state_dict = model_state_dict
        self.num_labels = num_labels
        self._model = BertForSequenceClassification.from_pretrained(pretrained_model_name=self.model_state_dict, num_labels=self.num_labels)
        
    # Feed forward function
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None):
        
        if labels is not None:
            loss, logits = self._model(input_ids, token_type_ids, attention_mask, labels)
            return loss, logits
        else:
            logits = self._model(input_ids, token_type_ids, attention_mask, labels=None)
            return logits
